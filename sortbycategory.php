   ...
   <link href="sticky-footer.css" rel="stylesheet">
    ...
    <body style="background: #F6FAFA;">
        ...
        <div class="col-sm-8 col-md-8">
            ...
            include 'connect.php';
            $catgry=$_GET['id'];
            $res=mysql_query("select topic.topic_name,topic.topic_by,topic.category,topic.topic_id,topic.date_time,users.user_id,users.fname from topic left join users on topic.topic_by=users.user_id where topic.category='$catgry'  order by topic.date_time desc");
            ...
            while($row=mysql_fetch_array($res))
            {
                ...
                echo "<p style='font-size:20px'><a href='question.php?id=$ques'>".$row['topic_name']."</a><br>";
                echo "<span style='font-size:15px'>in <b>".$row['category']."</b></span></p>";
                //fetching the ansers to the question which is most recent
                echo '</div>';
                $see_ans=  mysql_query("select reply.reply_content,reply.reply_id,reply.reply_by,users.user_id,users.fname from reply left join users on reply.reply_by=users.user_id where reply.reply_topic='$t_id' order by reply.date_time_r desc limit 1");
